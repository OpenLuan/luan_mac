...
#ifndef _WIN32
/*
** Push the target path of a symbolic link onto the stack (readlink(2)).
** Grows the buffer to fit arbitrarily long targets. Pushes nil on failure.
*/
static void push_link_target(lua_State *L, const char *file) {
	char *target = NULL;
	size_t cap = 256;
	for (;;) {
		char *nb = realloc(target, cap);
		if (!nb) { free(target); lua_pushnil(L); return; }
		target = nb;
		ssize_t n = readlink(file, target, cap);
		if (n < 0) { free(target); lua_pushnil(L); return; }
		if ((size_t)n < cap) { lua_pushlstring(L, target, (size_t)n); free(target); return; }
		cap *= 2; /* truncated: buffer too small, retry larger */
	}
}
#endif
...
static int copy_file (lua_State *L) {
	const char *afile = luaL_checkstring (L, -2);
    const char *bfile = luaL_checkstring (L, -1);
	int fail;
    ...
    outputFd = open(bfile, openFlags, filePerms);
    if (outputFd == -1){
        close(inputFd);
		lua_pushnil (L);
        lua_pushfstring (L, "%s", strerror(errno));
    ...
    while ((numRead = read(inputFd, buf, BUF_SIZE)) > 0){
        if (write(outputFd, buf, numRead) < numRead){
            if (close(inputFd) == -1 || close(outputFd) == -1){
                ...
                lua_pushnil (L);
                lua_pushfstring (L, "no more space on write");
                return 2;
            }
        }
    }
    ...
    if (numRead == -1) {
        lua_pushnil(L);
        lua_pushfstring (L, "error on write file");
        return 2;
    }
...
*/
static int _file_info_ (lua_State *L, int (*st)(const char*, STAT_STRUCT*), int is_lstat) {
	int i;
	STAT_STRUCT info;
	const char *file = luaL_checkstring (L, 1);
#ifndef _WIN32
...
#ifndef _WIN32
	is_link = is_lstat && S_ISLNK (info.st_mode);
#endif
	if (lua_isstring (L, 2)) {
		int v;
...
#ifndef _WIN32
		else if (strcmp (member, "target") == 0) {
			/* symlink target (lstat entry only); nil for non-links */
			if (is_link) push_link_target (L, file); else lua_pushnil (L);
			return 1;
		}
...
static int file_info (lua_State *L) {
	return _file_info_ (L, STAT_FUNC, 0);
}
...
static int link_info (lua_State *L) {
	return _file_info_ (L, LSTAT_FUNC, 1);
}
